ch. 1
    ...
    Использование instanceof
        ...
        проверка на класс: x instanceof Clazz
            может сразу давать ошибку компиляции, если между типом переменной x 
                и классом Clazz нет никакой связи
        ...
        проверка на интерфейс: x instanceof Interface
            всегда компилируется (кроме примитивных): подклассы могут реализовать интерфейс,
        ...
        часто применяется для проверки перед использованием cast
            если неподдерживаемые - вбрасывать исключение
        
        но лучше не использовать в обычной практике

    virtual method invocation
        просто прямое следствие переопределения методов: 
            при вызове методов не важен тип переменной на этапе компиляции,
            а важен тип объекта в рантайме
        ...
        с полями это не работает (сокрытие полей): результат зависит от типа переменной,
            от которой вызывается поле

    Аннотирование переопределенных методов
        аннотация @Override
        для указания, что программист переопределяет метод (класса или интерфейса)
        при указании @Override на методе, который не переопределяется - ошибка компиляции
    ...
    toString(), equals(), hashCode()
        ...
        equals
            ...
            требования к методу equals:
                (подразумевается x, y, z не равны null)
                - для любого x: x.equals(x) -> true
                - для любых x, y x.equals(y) должно возвращать true только если y.equals(x) 
                    возвращает true
                - для любых x,y,z: если x.equals(y)=true и y.equals(z)=true, тогда
                    x.equals(z) должно возвращать true
                - многократный вызов на одних и тех же объектах должен давать постоянный
        ...
        hashCode
            ...
            основные требования:
                - в рамках одной программы результат hashCode() не должен, 
                    т.е. в хеш не должны влючаться изменяемые поля
                - если equals() возвращает true для двух объектов, хеши этих
                    объектов должны совпадать, т.е. hashCode() должен включать те 
                    поля, которые используются в equals()
                - если equals() возвращает false для двух объектов, хеши этих не
    ...
    Перечисления (Enums)
        ...
        Основы 
            ...
            использование: через точку просто
            допускают сравнение через ==
            Season s = Season.WINTER;
            boolean t = s == Season.WINTER; // true

            метод values() возвращает массив элементов:
        ...
        Добавление конструкторов, полей, методов
            public enum Season {
            ...
            конструктор только private допустим: для перечисления конструктор вызывается только 
                внутри самого перечисления. public конструктор - ошибка компиляции
            конструкторы будут сразу вызваны для всех значений при первом обращении
                к одному из значений

            вызов публичных методов
                Season.WINTER.print();

            Внутри перечисления можно объявлять абстрактные методы, тогда 
                каждый  элемент должен переопределить
            ...
            можно объявить и реализовать метод, а внутри элемента переопределить
            внутри элемента можно объявить метод, но вызвать его нельзя

    Вложенные классы

        Основы
            вложенный класс - класс, определенный внутри другого класса
            внутренний класс - частный случай: не статический вложенный класс - 

            типы вложенных классов:
                - внутренний класс-член - класс, определенный на уровне переменных класса,
                    не статический
        ...
        Member Inner CLass 
            ...
            Создавать экземпляр внутреннего класса можно так:
                Outer outer = new Outer();
                Inner inner = outer.new Inner();
                причем это можно сделать внутри статического метода внешнего класса

            Можно объявлять переменные во внутреннем классе с таким же именем
                как во внешнем: работает сокрытие. Обратится к переменной внешнего
                класса из внутреннего можно по типу Outer.this.msg

            приватные интерфейсы: допустимы внутри класса, методы интерфейса конечно публичные
                такие интерфейсы вне класса не доступны

        Local Inner Class
            класс, определенный внутри метода
            область действия - только внутри метода: создавать экземпляры только внутри метода
            можно возвращать как результат метода ???
            ...
            свойства:
                ...
                - имеет доступ к локальным переменным метода, только если
                    они final или эффективно final (присваивание один раз 
                    или иначе: при добавлении final код остается корректным)
                    при компиляции такие переменные помещаются как поля класса
        ...
        Anonymous Inner Class
            ...
            объявляется сразу через "new"
            обязан переопределять существующий класс или реализовывать интерфейс
            одновременно и класс, и интерфейс или несколько интерфейсов нельзя
        ...
        Static Nested Class 
            ...
            не может иметь доступ к элементам внешнего класса без экземпляра класса 
                через экземпляр имеет доступ к любым, в т.ч. приватным
        ...
        Сводная таблица
            ...
            ===============================/==============/=============/ ===========/=================
            доступные модификаторы доступа /     любой    /  нет        /  нет       / любой     
            наслед-е классов/интерфейсов   /     любые    /  любые      /  один      / любые
            может быть абстрактным         /     да       /  да         /  N/A       / да  
            может быть final               /     да       /  да         /  N/A       / да  
            доступ к членам внешнего       /     да       /  да         /  да        / только через экземпляр
            доступ к локальным переменным  /     нет      /  эфф. final /  эфф. final/ нет
            статические методы             /     нет      /  нет        /  нет       / да

ch. 2 Design Patterns and Principles
    ...
    Реализация полиморфизма
        ...
        Object vs Reference
            ...
            смена типы ссылки может дать доступ к другим методам, хотя эти методы
                у объекта в памяти всегда есть
    ...
    Design Principles
        ...
        Инкапсуляция
            ...
            решается через объявление полей private и предоставления геттеров/сеттеров
            никто кроме самого класса, не должен иметь непосредственный доступ к данным класса

            обеспечивает поддержку инвариантов класса (свойства, утверждения о данных)

        JavaBeans
            соглашения об использовании полей:
        ...
        Отношение Is-a (является)
            свойство объекта быть экземпляров некого типа
            напр. класс Cat наследует класс Pet, тогда Cat Is-a Pet
                (класс Cat является классом Pet)

            можно проверять иерархию классов при разработке, задавая вопрос
                Is <subclass> a <superclass>?

        Отношение Has-a
            содержит ли объект некое свойство/значение
            причем если объект содержит protected или public поле,
                тогда все дочерние классы также содержит это поле

        Композиция объектов
            свойство класса содержать другой класс для использования методов этого класса
            по сути работает отношение Has-a
    ...
    Design Patterns
        ...
        Singleton Pattern
            создание объекта класса в единственном экземпляре для использования в 
                нескольких местах
            ...
            ключевые моменты:
                - обязателен приватный конструктор без параметров 
                    (запрещаем создавать экземпляры через конструкторы)
                    что делает этот класс финальным (дочерние не смогут вызвать super())
                - статическая финальная переменная для хранения единственного экземпляра
                - статический метод для получения этого экземпляра
                - инициализировать переменную instance можно не при объявлении, а
                    в статическом блоке

            Ленивая инициализация
                статическая инициализация происходит при загрузке класса в JVM
                можно отложить до первого вызова метода getInstance():
        ...
        Immutable Objects
            ...
            создание объектов, состояние которых не меняется после того, как
                объект был создан
            ...
            основные мероприятия:
                - все свойства объекта только через конструктор
                    ...
                    после создания объекта внешний код может их изменить
                    т.е. в объекте копировать внешние мутабельные объекты нужно
                - все переменные экземпляра должны быть private и final
                - не создаются сеттеры
                - не позволять, чтобы мутабельные изменяемые объекты-свойства
                    изменялись через методы или непосредственно (доступ к списку через геттер напр.)
                    иначе: нельзя давать ссылки на мутабельные объекты в  составе немутабельного
                    выход - возвращать копию, использовать неизменяемые коллекциии или
                        обеспечить итерацию
                - запретить переопределение методов
                    подклассы могут нарушить иммутабельность
                    выход: сделать класс финальным

        Builder Pattern
            как создать объект, требующий установки большого количества
                параметров при создании
            ...
            суть: использование объекта, в который параметры передаются постепенно
                через цепочку методов, а объект создается последним вызовом метода build()
        ...
        Factory Pattern 
            ...
            в основе использование класса-фабрики, который создает объекты
                на основе своих входных параметров
            ...
            в этом примере объекты можно создавать минуя фабрику
            что бы запретить это нужно помещать фабрику и объекты в один пакет
                и исключать публичные конструкторы, оставляя protected или default

ch. 3 Generics and Collections
    ...
    Универсальные типы
        ...
        Универсальные классы
            ...
            конвенции именования для формальных типов:
                заглавные одиночные буквы
                    E(element), K(key), V(value), N(number), T(общий тип), S, U, V, .. (для множественных)

        Стирание типов (Type Erasure)
            универсальные типы дают компилятору дополнительные возможности для проверки
            но во время компиляции все формальные типы заменяются на Object
            т.е. класс после компиляции: public class Foo {private Object someVar;}
        ...
        Универсальные интерфейсы
            ...
            три способа реализовать такой интерфейс:
                - указать конкретный тип в классе при реализации интерфейса, тогда
                и при реализации метода указываем конкретный тип
                ...
                - не указывать универсальный тип вообще
                    получим предупреждение компилятора
                    вместо универсального типа придется использовать Object
                     public class Clazz implements Bar{
                        void foo(Object t) {...}
                    }

        Что нельзя делать с универсальными типами
            (основные ограничения - следствие стирания типов)
            ...
            - нельзя использовать универсальный тип в статическом контексте 
                (все варианты будут преобразовываться к единственному случаю)
        ...
        legacy code, предупреждения компилятора

            код с использованием сырых (raw) типов без конкретизации типа:
                напр. List list = new ArrayList();
            в такой список можно вставить любой Object, но при извлечении
                в специализированный тип можем получить ClassCastException
        ...
        Границы типов
            ...
            wildcard без ограничений (подстановочные типы)
                ...
                если бы было можно, тогда проходило бы такое:
                    ...
                    List<Object> objs = numbers; // не пройдет, но если бы
                    ...
                    а с массивами:
                        ...
                        // с универсальными типами такого нет (стирание), поэтомк сразу запрещает
            ...
            wildcard с верхней границей
                ...
                // любой наследник Number или сам Number
                по прежнему при компиляции Object

                проблема с добавлением элементов:
                    в списки с wildcard, wildcard с верхней границей нельзя добавлять элементы
                    объект какого бы типа не добавляли, нельзя сказать что это будет 
                        соответствовать переданному списку
                    void foo(List<?> list) {
            ...
            wildcard с нижней границей
                ...
                // сам Integer или его родительские классы (Number, Object)
            ...
            общее:
                выражение типа <?>, <? extends T>, <? super U> не является типом,
                    поэтому нельзя напр. public <? extends String> foo() {..}
...
ch. 4 Функциональное программирование
...
ch. 5 Dates, String, Localization
...
ch. 8 IO
    ...
    Основы файлов / директорий
        ...
        Класс File
            ...
            сам не читает / не записывает данные

            создание объекта класса обычно с абсолютным или относительным путем
    ...
    Потоки
        ...
        Номерклатура потоков

            Байтовые потоки vs символьные потоки

                потоки, имеющие 'Stream' в названии, байтовые, читают/пишут 
                    любые данные или байты
                потоки, имеющие 'Reader/Writer' в названии, символьные, 
                    работают только с символами, строками 
            ...
            Low-level vs High-level
                ...
                низкоуровневые потоки непосредственно подключаются к источнику данных 
                    (файлу, строке и т.п.)  Напр. FileInputStream побайтово читает файл

                высокоуровневые потоки подключаются к другим потокам через обертывание

                напр. 
                BufferedReader br = new BufferedReader(new FileInputStream("1.txt"));

                в реальных проектах всегда использовать потоки с буферами. Не использовать
                    низкоуровневые FileInputStream / FileOutputStream



        Иерархия потоков

            в основе абстрактные классы InputStream, OutputStream, Reader, Writer
            наследники обычно имеют их в именах как суффиксы

            параметры в конструкторах высокоуровневых потоков имеют тип абстрактных классов
        ...
        Таблица потоков
            ...
            FileInputStream         байтовый файловый поток
            FileOutputStream
            FileReader              символьный файловый поток
            FileWriter

            BufferedReader          поток с буффером
            BufferedWriter
