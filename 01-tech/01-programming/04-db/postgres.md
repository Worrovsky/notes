...
- [2. Утилита psql](#2-Утилита-psql)
    ...
    - [2.6 Переменные psql](#26-Переменные-psql)
...
Запуск по умолчанию: отключить, управление????
...
В общем случае к данным системного каталога можно обращаться через SQL команды или команды psql (обычно с `d` начинаются)
